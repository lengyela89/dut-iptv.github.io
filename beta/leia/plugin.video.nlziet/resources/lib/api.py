...
def api_play_url(type, channel=None, id=None, video_data=None, from_beginning=0, pvr=0):
    ...
    if not type == 'vod' and (pvr == 0 or settings.getBool(key='ask_start_from_beginning') or from_beginning == 1):
        ...
        location_url = '{base_url}/v6/epg/location/{location}'.format(base_url=CONST_API_URL, location=id)

        download = api_download(url=location_url, type='get', headers=None, data=None, json_data=False, return_json=True)
        data = download['data']
        code = download['code']
        ...
        if check_key(info, 'VodContentId') and len(unicode(info['VodContentId'])) > 0:
            ...
            play_url = '{base_url}/v6/stream/handshake/Widevine/dash/VOD/{id}?playerName=NLZIET%20Meister%20Player%20Web&profile=default&maxResolution=&timeshift={timeshift}'.format(base_url=CONST_API_URL, id=info['VodContentId'], timeshift=timeshift)
            ...
            timeshift = 'false'
            play_url = '{base_url}/v6/stream/handshake/Widevine/dash/Restart/{id}?playerName=NLZIET%20Meister%20Player%20Web&profile=default&maxResolution=&timeshift={timeshift}'.format(base_url=CONST_API_URL, id=id, timeshift=timeshift)
